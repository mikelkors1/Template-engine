...
struct TestCase {
    ...
    TestCase(const std::string& testName, bool isNegative = false)
        : name(testName)
        , templateFile((fs::path("..") / "tests" / "test_cases" / testName / "template.txt").string())
        , dataFile((fs::path("..") / "tests" / "test_cases" / testName / "data.dat").string())
        , expectedFile((fs::path("..") / "tests" / "test_cases" / testName / "expected.txt").string())
